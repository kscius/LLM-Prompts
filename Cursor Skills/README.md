...
**Last updated:** 2026-03-30 (`exported-cursor-skills`: **39** skills [`robocopy /MIR` desde `%USERPROFILE%\.cursor\skills\`]; `exported-agents-skills`: **find-skills** únicamente [espejo fiel de `%USERPROFILE%\.agents\skills\`]; manifest = `%USERPROFILE%\.cursor\skills-cursor\.cursor-managed-skills-manifest.json`)
